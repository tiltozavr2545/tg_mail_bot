    ...
    def __init__(self, cfg: Config) -> None:
        ...
        if cfg.tracked_people:
            ...
            instruction.append(
                ...
                f"{people}. КАЖДОГО человека выводи с новой строки — ровно одна строка "
                "на человека (здесь переносы строк нужны, в отличие от обычных писем). "
                "Для каждого укажи его баллы по каждому заданию и ОБЯЗАТЕЛЬНО из скольки "
                "максимум, если максимум указан (например, в заголовке столбца вроде "
                "«ЛР1 (из 10)» — тогда пиши «9 из 10»). Учитывай сокращения и разный "
                "порядок ФИО. Кого нет в таблице — просто пропусти, не упоминай."
            )
