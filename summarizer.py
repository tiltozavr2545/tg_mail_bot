...
def _build_prompt(email: Email) -> str:
    ...
    for att in email.attachments:
        if att.markdown:
            ...
            parts.append("")
            parts.append(f"=== Приложен файл «{att.filename}» (содержимое не анализировалось) ===")
    return "\n".join(parts)
...
class Summarizer:
    def __init__(self, cfg: Config) -> None:
        ...
        instruction.append(
            ...
            "сделать."
            "\n— Если у письма есть приложенный файл, содержимое которого не "
            "анализировалось (лекция, книга, презентация, документ и т.п.), не пытайся "
            "пересказать его — просто упомяни, что такой файл приложен (по названию)."
